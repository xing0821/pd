...
func (s *regionTestSuite) TestRegion(c *C) {
	...
	c.Assert(err, IsNil)
	regionsInfo := apiregion.RegionsInfo{}
	...
	_, output, err = pdctl.ExecuteCommandC(cmd, args...)
	c.Assert(err, IsNil)
	regionInfo := apiregion.RegionInfo{}
	c.Assert(json.Unmarshal(output, &regionInfo), IsNil)
	region := leaderServer.GetRegionInfoByID(1)
	c.Assert(apiregion.NewRegionInfo(region), DeepEquals, &regionInfo)
	...
	regionsInfo = apiregion.RegionsInfo{}
	c.Assert(json.Unmarshal(output, &regionsInfo), IsNil)
	regions = apiregion.TopNRegions(leaderServer.GetRegions(), func(a, b *core.RegionInfo) bool { return a.GetBytesRead() < b.GetBytesRead() }, 2)
	pdctl.CheckRegionsInfo(c, regionsInfo, regions)
	...
	regionsInfo = apiregion.RegionsInfo{}
	c.Assert(json.Unmarshal(output, &regionsInfo), IsNil)
	regions = apiregion.TopNRegions(leaderServer.GetRegions(), func(a, b *core.RegionInfo) bool { return a.GetBytesWritten() < b.GetBytesWritten() }, 2)
	pdctl.CheckRegionsInfo(c, regionsInfo, regions)
	...
	regionsInfo = apiregion.RegionsInfo{}
	c.Assert(json.Unmarshal(output, &regionsInfo), IsNil)
	regions = apiregion.TopNRegions(leaderServer.GetRegions(), func(a, b *core.RegionInfo) bool {
	...
	regionInfo = apiregion.RegionInfo{}
	c.Assert(json.Unmarshal(output, &regionInfo), IsNil)
	c.Assert(&regionInfo, DeepEquals, apiregion.NewRegionInfo(r2))
