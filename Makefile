...
RETOOL_DIR := $$PWD/_retools

GOFAIL_ENABLE  := $$(find $$PWD/ -type d | grep -vE "(\.git|vendor)" | xargs retool -tool-dir $(RETOOL_DIR) do gofail enable)
GOFAIL_DISABLE := $$(find $$PWD/ -type d | grep -vE "(\.git|vendor)" | xargs retool -tool-dir $(RETOOL_DIR) do gofail disable)
...
check-fail:
	CGO_ENABLED=0 retool -tool-dir $(RETOOL_DIR) do gometalinter.v2 --disable-all \
	  --enable errcheck \
	  $$($(PACKAGE_DIRECTORIES))
	CGO_ENABLED=0 retool -tool-dir $(RETOOL_DIR) do gosec $$($(PACKAGE_DIRECTORIES))
...
retool-setup:
	...
	@retool -tool-dir $(RETOOL_DIR) sync
...
static:
	...
	retool -tool-dir $(RETOOL_DIR) do govet --shadow $$($(PACKAGE_DIRECTORIES)) 2>&1 | $(GOCHECKER)

	CGO_ENABLED=0 retool -tool-dir $(RETOOL_DIR) do gometalinter.v2 --disable-all --deadline 120s \
	  --enable misspell \
	  --enable megacheck \
	  --enable ineffassign \
	  $$($(PACKAGE_DIRECTORIES))
...
lint:
	@echo "linting"
	CGO_ENABLED=0 retool -tool-dir $(RETOOL_DIR) do revive -formatter friendly -config revive.toml $$($(PACKAGES))
