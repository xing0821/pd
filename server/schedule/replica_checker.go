...
func (r *ReplicaChecker) Check(region *core.RegionInfo) *Operator {
	...
	if len(region.GetPeers()) < r.cluster.GetMaxReplicas() && r.cluster.IsMakeUpReplicaEnabled() {
		log.L().Debug("region has fewer than max replicas", zap.Uint64("region", region.GetID()), zap.Int("peers", len(region.GetPeers())))
	...
	if len(region.GetVoters()) > r.cluster.GetMaxReplicas() && r.cluster.IsRemoveExtraReplicaEnabled() {
		log.L().Debug("region has more than max replicas", zap.Uint64("region", region.GetID()), zap.Int("peers", len(region.GetPeers())))
...
func (r *ReplicaChecker) selectBestPeerToAddReplica(region *core.RegionInfo, filters ...Filter) (*metapb.Peer, float64) {
	...
	if storeID == 0 {
		log.L().Debug("no best store to add replica", zap.Uint64("region", region.GetID()))
		return nil, 0
	}
...
func (r *ReplicaChecker) selectWorstPeer(region *core.RegionInfo) (*metapb.Peer, float64) {
	...
	if worstStore == nil {
		log.L().Debug("no worst store", zap.Uint64("region", region.GetID()))
		return nil, 0
	}
...
func (r *ReplicaChecker) checkDownPeer(region *core.RegionInfo) *Operator {
	...
	for _, stats := range region.GetDownPeers() {
		...
		if store == nil {
			log.L().Info("lost the store, maybe you are recovering the PD cluster", zap.Uint64("store", peer.GetStoreId()))
			return nil
		}
...
func (r *ReplicaChecker) checkBestReplacement(region *core.RegionInfo) *Operator {
	...
	if newScore <= oldScore {
		log.L().Debug("no better peer", zap.Uint64("region", region.GetID()), zap.Float64("new score", newScore), zap.Float64("old score", oldScore))
