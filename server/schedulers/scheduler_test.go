...
func (s *testShuffleLeaderSuite) TestShuffle(c *C) {
	...
	sl, err := CreateScheduler("shuffle-leader", schedule.NewOperatorController(nil, nil))
	c.Assert(err, IsNil)
...
func (s *testBalanceAdjacentRegionSuite) TestBalance(c *C) {
	...
	sc, err := CreateScheduler("adjacent-region", schedule.NewOperatorController(nil, nil), "32", "2")
	c.Assert(err, IsNil)
...
func (s *testBalanceAdjacentRegionSuite) TestNoNeedToBalance(c *C) {
	...
	sc, err := CreateScheduler("adjacent-region", schedule.NewOperatorController(nil, nil))
	c.Assert(err, IsNil)
	c.Assert(sc.Schedule(tc), IsNil)
...
func (s *testRejectLeaderSuite) TestRejectLeader(c *C) {
	...
	sl, err := CreateScheduler("label", oc)
	c.Assert(err, IsNil)
	op := sl.Schedule(tc)
	...
	// any operators.
	bs, err := CreateScheduler("balance-leader", oc)
	c.Assert(err, IsNil)
	op = bs.Schedule(tc)
	c.Assert(op, IsNil)

	// Can't evict leader from store2, neither.
	el, err := CreateScheduler("evict-leader", oc, "2")
	c.Assert(err, IsNil)
	op = el.Schedule(tc)
	c.Assert(op, IsNil)
...
func (s *testShuffleHotRegionSchedulerSuite) TestBalance(c *C) {
	...
	tc := mockcluster.NewCluster(opt)
	hb, err := CreateScheduler("shuffle-hot-region", schedule.NewOperatorController(nil, nil))
	c.Assert(err, IsNil)
...
func (s *testEvictLeaderSuite) TestEvictLeader(c *C) {
	...
	sl, err := CreateScheduler("evict-leader", schedule.NewOperatorController(nil, nil), "1")
	c.Assert(err, IsNil)
...
func (s *testShuffleRegionSuite) TestShuffle(c *C) {
	...
	sl, err := CreateScheduler("shuffle-region", schedule.NewOperatorController(nil, nil))
	c.Assert(err, IsNil)
	c.Assert(sl.IsScheduleAllowed(tc), IsTrue)
	c.Assert(sl.Schedule(tc), IsNil)
