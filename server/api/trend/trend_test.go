...
func (s *testTrendSuite) TestTrend(c *C) {
	svr, cleanup := helper.MustNewServer(c)
	defer cleanup()
	helper.MustWaitLeader(c, []*server.Server{svr})

	helper.MustBootstrapCluster(c, svr)
	for i := 1; i <= 3; i++ {
		helper.MustPutStore(c, svr, uint64(i), metapb.StoreState_Up, nil)
	}
	...
	helper.MustRegionHeartbeat(c, svr, region4)
	helper.MustRegionHeartbeat(c, svr, region5)
	helper.MustRegionHeartbeat(c, svr, region6)
	...
	helper.MustRegionHeartbeat(c, svr, region4.Clone(core.WithLeader(region4.GetStorePeer(2))))
	...
	var trend Trend
	err = util.ReadJSONWithURL(fmt.Sprintf("%s/pd/api/v1/trend", svr.GetAddr()), &trend)
	c.Assert(err, IsNil)
