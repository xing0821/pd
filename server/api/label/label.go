...
import (
	...
	"github.com/pingcap/pd/server/api/store"
...
func (h *labelsHandler) GetStores(w http.ResponseWriter, r *http.Request) {
	...
	stores := cluster.GetStores()
	storesInfo := &store.StoresInfo{
		Stores: make([]*store.StoreInfo, 0, len(stores)),
	}
	...
	for _, s := range stores {
		...
		storeInfo := apistore.NewStoreInfo(h.svr.GetScheduleConfig(), store)
