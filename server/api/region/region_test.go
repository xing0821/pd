...
func (s *testRegionSuite) TestRegionCheck(c *C) {
	...
	r3 := &RegionsInfo{}
	err = util.ReadJSONWithURL(url, r3)
	c.Assert(err, IsNil)
...
func (s *testRegionSuite) TestRegions(c *C) {
	...
	err := util.ReadJSONWithURL(url, RegionsInfo)
	c.Assert(err, IsNil)
...
func (s *testRegionSuite) TestStoreRegions(c *C) {
	...
	helper.MustRegionHeartbeat(c, s.svr, r1)
	helper.MustRegionHeartbeat(c, s.svr, r2)
	helper.MustRegionHeartbeat(c, s.svr, r3)

	regionIDs := []uint64{2, 3}
	url := fmt.Sprintf("%s/regions/store/%d", s.urlPrefix, 1)
	r4 := &RegionsInfo{}
	err := util.ReadJSONWithURL(url, r4)
	c.Assert(err, IsNil)
	...
	r5 := &RegionsInfo{}
	err = util.ReadJSONWithURL(url, r5)
	c.Assert(err, IsNil)
	...
	r6 := &RegionsInfo{}
	err = util.ReadJSONWithURL(url, r6)
	c.Assert(err, IsNil)
...
func (s *testRegionSuite) checkTopRegions(c *C, url string, regionIDs []uint64) {
	regions := &RegionsInfo{}
	err := util.ReadJSONWithURL(url, regions)
	c.Assert(err, IsNil)
...
func (s *testGetRegionSuite) TestRegionKey(c *C) {
	...
	err := util.ReadJSONWithURL(url, RegionInfo)
	c.Assert(err, IsNil)
