...
func (h *storeHandler) Get(w http.ResponseWriter, r *http.Request) {
	...
	if errParse != nil {
		util.ErrorResp(h.rd, w, errcode.NewInvalidInputErr(errParse))
		return
	}
...
func (h *storeHandler) Delete(w http.ResponseWriter, r *http.Request) {
	cluster := h.GetRaftCluster()
	if cluster == nil {
		util.ErrorResp(h.rd, w, errcode.NewInternalErr(server.ErrNotBootstrapped))
		return
	}
...
func (h *storeHandler) SetLabels(w http.ResponseWriter, r *http.Request) {
	...
	if err := server.ValidateLabels(labels); err != nil {
		util.ErrorResp(h.rd, w, errcode.NewInvalidInputErr(err))
		return
	}
