...
func (s *testStoreSuite) TestStoreSetState(c *C) {
	...
	// Set to Offline.
	info = StoreInfo{}
	err = util.PostJSON(url+"/state?state=Offline", nil)
	c.Assert(err, IsNil)
	err = util.ReadJSONWithURL(url, &info)
	c.Assert(err, IsNil)
	...
	// Invalid state.
	info = StoreInfo{}
	err = util.PostJSON(url+"/state?state=Foo", nil)
	c.Assert(err, NotNil)
	err = util.ReadJSONWithURL(url, &info)
	c.Assert(err, IsNil)
	c.Assert(info.Store.State, Equals, metapb.StoreState_Offline)
	...
	// Set back to Up.
	info = StoreInfo{}
	err = util.PostJSON(url+"/state?state=Up", nil)
	c.Assert(err, IsNil)
	err = util.ReadJSONWithURL(url, &info)
	c.Assert(err, IsNil)
	c.Assert(info.Store.State, Equals, metapb.StoreState_Up)
